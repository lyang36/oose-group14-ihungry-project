...
public class LoginActivity extends Activity {
	...
	/**
	 * A call-back for when the user presses the login button.<p>
	 * The combination of username and password is sent to the server for login verification.<p>
	 * A customer instance is returned if login successfully.<p>
	 * A signal to switch to main Screen is sent to ActivityController if the username and pwd are valid.
	 */
	OnClickListener btn_Login_Listener = new OnClickListener() {
		public void onClick(View v) {
			Customer myCust = clientModel.getCustomerInfo(et_username.getText().toString(), 
			...
			if( myCust != null ){
				...
				String realName = SignupActivity.handleEditText(cInfo.getRealName());
				String address = SignupActivity.handleEditText(cInfo.getAddress().getAddress());
				String primPhone = SignupActivity.handleEditText(cInfo.getPrimPhone());
				String secPhone = SignupActivity.handleEditText(cInfo.getSecPhone());
				String email = SignupActivity.handleEditText(cInfo.getEmail());
				String birth = SignupActivity.handleEditText(cInfo.getBirthDate());
				...
				ToastDisplay.DisplayToastOnScr(LoginActivity.this, "Connection Error or Login Failed");		
			}		
		}
	};
