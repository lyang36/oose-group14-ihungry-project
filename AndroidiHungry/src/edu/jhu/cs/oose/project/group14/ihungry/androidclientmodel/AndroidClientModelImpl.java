...
public class AndroidClientModelImpl implements AndroidClientModel {
	...
	public List<Order> retrieveAllOrders(String custId){
		/* Generate a sending message containing custId to server
		 * and if response are valid => convert to a List<Order> object */
	...
	public List<Order> retrieveOrders(String custId, String status,
			int count) {
		/* 1. Generate a sending message to be sent to the server with Customer, status and count */
		/* 2. Call internetClient.sendAndGet to get response from server */
